...
RSpec.configure do |config|
  ...
  config.before(:all) do
    ...
    when 'mobile_web'
      @homepage = setup.load_browser
    when 'mobile_ios'
      @homepage = setup.load_appium
    when 'mobile_android'
      @homepage = setup.load_appium
    else
      raise Exception, "[ERROR] Need to provide platform in environment.yaml in valid form: web, mobile_web, mobile_ios or mobile_android"
    end

    @rspec_reporter = RSpec.configuration.reporter

    @is_fast_fail = setup.fast_fail['active']
    @fast_fail_steps = setup.fast_fail['count']
    puts "Running tests with fast fail option on (#{@fast_fail_steps} steps)" if @is_fast_fail
  end

  config.after(:each) do |example|
    if setup.screenshots_enabled
  ...
  if setup.retry['active']
    config.verbose_retry = true
    config.default_retry_count = setup.retry['count']
    config.default_sleep_interval = setup.retry['sleep']
    config.exceptions_to_retry = [Net::ReadTimeout, Selenium::WebDriver::Error::ServerError, Selenium::WebDriver::Error::UnknownError, Selenium::WebDriver::Error::NoSuchElementError, Selenium::WebDriver::Error::SessionNotCreatedError]
  end
